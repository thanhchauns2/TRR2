...
vector<vec(ll)>  Nhan(vector<vec(ll)> a, vector<vec(ll)> b)
{
	vector<vec(ll)>  token = a;
	FOR(i,0,a.size()-1)
	{
		FOR(j,0,a.size()-1)
		{
...
vector<vec(ll)> LuyThua(vector<vec(ll)> a, ll b)
{
	if (b == 1) return a;
	vec(vec(ll)) K = LuyThua(a,b/2);
	if (b&1) return Nhan(K,Nhan(K,a));
	return Nhan(K,K);
}
